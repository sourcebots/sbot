...
def test_board():
    ...
    try:
        ...
        for motor in range(2):
            ...
            # test off current
            output_off_current = board.motors[motor].power
            logger.info(
            ...
            for direction in (1, -1):
                for abs_power in range(0, 1000, 100):
                    power = abs_power * direction
                    test_step = f"motor {motor}, {power / 10:.0f}% power"
                    logger.info(f"Testing {power / 10:.0f}% power")

                    expected_out_current = (input_voltage / MOTOR_RESISTANCE) * (abs_power / 1000)  # noqa
